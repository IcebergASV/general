...
// #include <yolov8_msgs/msg/iceberg_detection.hpp>
// #include <yolov8_msgs/msg/iceberg_detection_array.hpp>
// #include <yolov8_msgs/msg/iceberg_bounding_box2_d.hpp>
...
//         det_pub_ = this->create_publisher<yolov8_msgs::msg::IcebergDetectionArray>(output_det_topic_, 10);
...
//         yolov8_msgs::msg::IcebergDetectionArray det_array;
...
//                 yolov8_msgs::msg::IcebergBoundingBox2D bbox;
//                 bbox.xmin = rect.x;
//                 bbox.ymin = rect.y;
//                 bbox.xmax = rect.x + rect.width;
//                 bbox.ymax = rect.y + rect.height;

//                 yolov8_msgs::msg::IcebergDetection det;
//                 det.class_id = 100;
//                 det.class_name = class_name_;
//                 det.score = 1.0;
//                 det.bbox = bbox;

//                 det_array.icebergdetections.push_back(det);
//             }
//         }
...
//     rclcpp::Publisher<yolov8_msgs::msg::IcebergDetectionArray>::SharedPtr det_pub_;
...
public:
    ColorDetector() : Node("configurable_color_filter") {
        ...
        this->declare_parameter("output_class_name", "color_object");
        this->declare_parameter("min_area", 75);
        this->declare_parameter("lower_hsv_list", std::vector<int64_t>{0, 100, 100, 160, 100, 100});
        this->declare_parameter("upper_hsv_list", std::vector<int64_t>{10, 255, 255, 180, 255, 255});
        ...
        if (lower_hsv.size() % 3 != 0 || upper_hsv.size() % 3 != 0 || lower_hsv.size() != upper_hsv.size()) {
            RCLCPP_ERROR(this->get_logger(), "Invalid HSV parameter format");
            rclcpp::shutdown();
            return;
        }
...
private:
    void image_callback(const sensor_msgs::msg::Image::SharedPtr msg) {
        ...
        try {
            auto filtered_msg = cv_bridge::CvImage(msg->header, "bgr8", cv_ptr->image).toImageMsg();
            img_pub_->publish(*filtered_msg);
            det_pub_->publish(det_array);
        } catch (const cv_bridge::Exception& e) {
            RCLCPP_ERROR(this->get_logger(), "Publishing error: %s", e.what());
