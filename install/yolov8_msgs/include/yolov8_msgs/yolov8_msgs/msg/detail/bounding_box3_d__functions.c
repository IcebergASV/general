  ...
  yolov8_msgs__msg__BoundingBox3D__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    yolov8_msgs__msg__BoundingBox3D * data =
      (yolov8_msgs__msg__BoundingBox3D *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!yolov8_msgs__msg__BoundingBox3D__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          yolov8_msgs__msg__BoundingBox3D__fini(&output->data[i]);
        }
        return false;
      }
    }
